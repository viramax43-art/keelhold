...
local CommissionMapConfig = {
	...
	Anchors = {
		...
		Leaderboard = {
			...
			MaxDistance = 10,
			PromptAtBase = true,
			PromptAnchorName = "BridgeDefensePromptAnchorLB",
			PromptWorldOffset = Vector3.new(7, 0, 0),
			PromptExclusivity = "OnePerButton",
		},
		DailyReward = {
		...
		Promocode = {
			-- Вторая доска WoodenLeaderboard (~22 studs по Z от первой)
			Prefer = { "WoodenLeaderboard" },
			Pick = "SecondClosestToSpawn",
			ObjectText = "Промокод",
			...
			MaxDistance = 10,
			PromptAtBase = true,
			PromptAnchorName = "BridgeDefensePromptAnchorPromo",
			PromptWorldOffset = Vector3.new(7, 0, 0),
			PromptExclusivity = "OnePerButton",
		},
	},
